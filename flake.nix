{
  description = "remote-dev binary and HostService artifacts";
  ...
  outputs = { self, nixpkgs, flake-utils, disko }:
    let
      version = "host-service-test.20260701T160745Z-g707922b62853-remote-dev-bin-test-707922b62853-1782921788";
      ...
      hostArtifacts = {
        ...
        agent_runtime_specs = {
          aarch64 = "host-runtime-specs/aarch64.json";
        };
        inherit version;
      };

      agentRuntimeSpecs = {
        aarch64 = builtins.fromJSON (builtins.readFile ./host-runtime-specs/aarch64.json);
      };
      ...
      mkTakeoverRunner = pkgs:
        pkgs.writeShellApplication {
          ...
          text = ''
            ...
            report_failure() {
              ...
              if [ "$status" -ne 0 ]; then
                if [ -n "$failure_detail" ]; then
                  ...
                  last_error="exit status: $status"
                fi
                post_event firstboot "$current_phase" "firstboot registration failed; retrying" "$last_error"
              fi
            }
            ...
            prepare_id="$(cat /etc/remote-dev/firstboot-prepare-id)"
            callback_token="$(cat /etc/remote-dev/firstboot-callback-token)"
            fallback_ip="$(cat /etc/remote-dev/bootstrap-public-ip 2>/dev/null || true)"
            ...
            http_status="$(curl -sS --connect-timeout 10 --max-time 60 \
              ...
              "$base_url/v1/host-prepares/$prepare_id/firstboot")"
            curl_status="$?"
            set -e
            if [ "$curl_status" -ne 0 ]; then
              [ -n "$http_status" ] || http_status=unavailable
              body="$(tr '\n' ' ' < "$tmp" | cut -c1-500 || true)"
              failure_detail="curl exit status: $curl_status; http_status=$http_status; body=$body"
              echo "remote-dev firstboot register: $failure_detail" >&2
              exit "$curl_status"
            fi
            case "$http_status" in
              2*) ;;
              *)
                body="$(tr '\n' ' ' < "$tmp" | cut -c1-500 || true)"
                failure_detail="http_status=$http_status; body=$body"
                echo "remote-dev firstboot register: $failure_detail" >&2
                exit 22
                ;;
            esac
...

            {
              ...
              users.users.root.hashedPassword = "$1$rdroot$DYmS1f6QxQhrWy7GGmAar/";
            ...
            write_install_proof() {
              ...
              jq -n \
                ...
                --arg image_contract_id "$(field image_contract_id)" \
                --argjson disk_count "$(field disk_count)" \
    ...
    in
    {
      ...
      lib.agentRuntimeSpecs = agentRuntimeSpecs;
      lib.agentRuntimeSpec = agentRuntimeSpecs.x86_64 or agentRuntimeSpecs.aarch64;
      nixosConfigurations.remote-dev-kexec-installer = remoteDevKexecSystem;

      nixosModules.remote-dev-host = { lib, pkgs, ... }:
        let
          system = pkgs.stdenv.hostPlatform.system;
          remoteDevPackage = self.packages.${system}.remote-dev or
            (throw "remote-dev artifact is not published for ${system} in this remote-dev-bin ref");
          runtimePackage = self.packages.${system}.remote-dev-runtime or
            (throw "remote-dev-runtime artifact is not published for ${system} in this remote-dev-bin ref");
        in
        {
          ...
          environment.systemPackages = [
            (mkAgentRuntimePackage pkgs remoteDevPackage runtimePackage)
            (hostGroupPackages system pkgs)."host-base-tools"
          ];
      ...
      nixosModules.remote-dev-firstboot-register = { pkgs, ... }:
        let
          firstbootRegisterScript = pkgs.writeShellScript "remote-dev-firstboot-register" ''
            set -euo pipefail
            mkdir -p /var/log/remote-dev
            exec > >(tee -a /var/log/remote-dev/firstboot-register.log) 2>&1
            marker=/var/lib/remote-dev/firstboot-register.done
        ...
        in
        {
          environment.etc."remote-dev/firstboot-register.sh" = {
            source = firstbootRegisterScript;
            mode = "0700";
          };
          ...
          systemd.services.remote-dev-firstboot-register = {
            ...
            wants = [ "network-online.target" ];
            after = [ "network-online.target" "sshd.service" ];
            path = [ pkgs.bash pkgs.coreutils pkgs.curl ];
            unitConfig.StartLimitIntervalSec = 0;
            serviceConfig = {
              Type = "oneshot";
              Restart = "on-failure";
    ...
    flake-utils.lib.eachDefaultSystem (system:
      ...
      in
      {
        packages = lib.optionalAttrs (system == "aarch64-linux") rec {
  remote-dev = mkLocalBinaryPackage
    pkgs
    "remote-dev"
    "remote-dev"
    ./artifacts/remote-dev-aarch64-linux.tar.gz;
  remote-dev-runtime = mkLocalBinaryPackage
    pkgs
    "remote-dev-runtime"
    "remote-dev-runtime"
    ./artifacts/remote-dev-runtime-aarch64-linux.tar.gz;
  remote-dev-agent-runtime = mkAgentRuntimePackage pkgs remote-dev remote-dev-runtime;
  "remote-dev-host-group-host-base-tools" = (hostGroupPackages system pkgs)."host-base-tools";
  "remote-dev-host-group-git-core" = (hostGroupPackages system pkgs)."git-core";
  "remote-dev-host-group-mosh-transport" = (hostGroupPackages system pkgs)."mosh-transport";
  "remote-dev-host-group-default-dev-shell-prefill" = (hostGroupPackages system pkgs)."default-dev-shell-prefill";
  "remote-dev-host-group-nix-source-baseline" = (hostGroupPackages system pkgs)."nix-source-baseline";
  "remote-dev-host-group-shell-startup" = (hostGroupPackages system pkgs)."shell-startup";
  "remote-dev-host-group-shell-extras" = (hostGroupPackages system pkgs)."shell-extras";
  "remote-dev-host-group-vscode-compat" = (hostGroupPackages system pkgs)."vscode-compat";
  "remote-dev-host-group-compression-tools" = (hostGroupPackages system pkgs)."compression-tools";
  "remote-dev-host-group-build-baseline" = (hostGroupPackages system pkgs)."build-baseline";
  "remote-dev-host-group-c-toolchain-gcc" = (hostGroupPackages system pkgs)."c-toolchain-gcc";
  "remote-dev-host-group-c-toolchain-clang" = (hostGroupPackages system pkgs)."c-toolchain-clang";
  "remote-dev-host-group-dev-diagnostics" = (hostGroupPackages system pkgs)."dev-diagnostics";
